...
public class BrightcovePlayerAccount
  implements
    OfflineVideoDownloadSession.OnOfflineVideoDownloadSessionListener {
  ...
  private List<
    OfflineVideoDownloadSession
  > offlineVideoDownloadSessions = new ArrayList<>();
  private boolean getOfflineVideoStatusesRunning = false;
  private List<
    Promise
  > getOfflineVideoStatusesPendingPromises = new ArrayList<>();
  ...
  public BrightcovePlayerAccount(
    final ReactApplicationContext context,
    final String accountId,
    final String policyKey,
    OnBrightcovePlayerAccountListener listener
  ) {
    ...
    this.offlineCatalog.findAllVideoDownload(
      ...
      new OfflineCallback<List<Video>>() {
        ...
        public void onSuccess(List<Video> videos) {
          for (Video video : videos) {
            OfflineVideoDownloadSession session = new OfflineVideoDownloadSession(
              context,
              accountId,
              policyKey,
              BrightcovePlayerAccount.this
            );
  ...
  public void requestDownloadWithReferenceId(
    ...
    this.listener.onOfflineStorageStateChanged(
      collectNativeOfflineVideoStatuses()
    );
  }
  ...
  public void deleteOfflineVideo(final String videoId, final Promise promise) {
    try {
      ...
      for (int i = this.offlineVideoDownloadSessions.size() - 1; i >= 0; i--) {
        OfflineVideoDownloadSession session = this.offlineVideoDownloadSessions.get(
          i
        );
      ...
      this.offlineCatalog.deleteVideo(
        ...
        new OfflineCallback<Boolean>() {
          ...
          public void onSuccess(Boolean aBoolean) {
            ...
            listener.onOfflineStorageStateChanged(
              collectNativeOfflineVideoStatuses()
            );
          }
  ...
  public void getPlaylistWithReferenceId(
    String referenceId,
    final Promise promise
  ) {
  ...
  private NativeArray collectNativePlaylist(
    String accountId,
    Playlist playlist
  ) {
  ...
  private boolean hasOfflineVideoDownloadSessionWithReferenceId(
    String referenceId
  ) {
    for (OfflineVideoDownloadSession session : this.offlineVideoDownloadSessions) {
      if (referenceId.equals(session.referenceId))
      return true;
    }
    return false;
  }
